...
from PS_lexer import token_types

inputTokens = token_types
# print(inputTokens)

def ignoreComments(code_string):
    # Eliminar los tokens ILCOMM y MLCOMM del string
    code_string = code_string.replace("ILCOMM ", "")  # Asegúrate de incluir un espacio después del token para evitar dejar espacios adicionales
...
def traceParsing(input_string, terminals, nonTerminals, table_path):
    ...
    with open('tableParse.csv', 'w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
    ...
    # Imprime en consola si la entrada fue aceptada o no
    if accepted:
        print("Accepted")
    else:
        print("Rejected")
...
print(traceParsing(input, terminals, nonTerminals, table_path))